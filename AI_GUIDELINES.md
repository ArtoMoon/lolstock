# AI Guidelines - Next.js & MongoDB Riot API Account Dashboard
...
* **Project Purpose:** Next.js-based personal alt-account manager & sync dashboard for League of Legends accounts using Riot Games API and MongoDB.
...
* **Data Schemas:** Maintain a clean `Account` Mongoose schema storing `riotId`, `puuid`, `level`, `rank`, `lastMatchId`, `status` (`available`, `archived`, `active`), and `lastCheckedAt`.

---

## 3. Strict Riot API & Rate-Limit Protocol
* **Rate Limits:** Respect the 20 req/10s and 100 req/2min limits.
* **Batch Delay Loop:** When iterating through accounts for a full account sync, enforce an explicit `await delay(1500)` (1.5–2.0 seconds) between requests to prevent HTTP 429 errors.
...
* **Database-First Caching Strategy:**
  ...
  * **Match History (`match-v5`):** Fetch only the latest match ID (`count=1`). If `lastMatchId` changes or level spikes, flag status as `active`.
